	   ...
	   u_int32_t mark_mask, u_int32_t mark_value)
{
	...
	if (sk && nf_tproxy_sk_is_transparent(sk)) {
		...
		pr_debug("redirecting: proto %hhu %pI4:%d -> %pI4:%d, mark: %x\n",
	...
	pr_debug("no socket, dropping: proto %hhu %pI4:%d -> %pI4:%d, mark: %x\n",
...
tproxy_tg6_v1(struct sk_buff *skb, const struct xt_action_param *par)
{
	...
	if (sk && nf_tproxy_sk_is_transparent(sk)) {
		...
		pr_debug("redirecting: proto %d %pI6:%d -> %pI6:%d, mark: %x\n",
	...
	pr_debug("no socket, dropping: proto %d %pI6:%d -> %pI6:%d, mark: %x\n",
		 tproto, &iph->saddr, ntohs(hp->source),
		 &iph->daddr, ntohs(hp->dest), skb->mark);
